...
<%
    ...
    int totalPage = 0; //总页数
    con.startCon(); 
    String condition = "select count(*) from commentCouInfo inner join courseInfo on commentCouInfo.couNum=courseInfo.couNum " +
...
<!-- 判断用户是不是直接打开这个网址，而有没有通过登录界面登录 -->
		...
		<center>
               ...
               <a href="admin/commentCourse/showCommentCouInfo.jsp?pageNo=1&couTerm=<%=couTerm %>">首页</a> 
               <a href="admin/commentCourse/showCommentCouInfo.jsp?pageNo=<%=pageNo-1 %>&couTerm=<%=couTerm %>">上一页</a> 
               <a href="admin/commentCourse/showCommentCouInfo.jsp?pageNo=<%=pageNo+1 %>&couTerm=<%=couTerm %>">下一页</a> 
               <a href="admin/commentCourse/showCommentCouInfo.jsp?pageNo=<%=totalPage %>&couTerm=<%=couTerm %>">末页</a>
        </center>
