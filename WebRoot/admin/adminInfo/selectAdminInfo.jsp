...
<%
    ...
    con.startCon();
    int rowCount = con.getCount("select count(*) from adminInfo where "+select+" like '%"+userInfo+"%'"); //获取总行数
    int totalPage = 0; //总页数
...
<html>
	...
	<input type="submit" value="搜索" class="btn btn-success" />
		...
		<center>
               ...
               <a href="admin/adminInfo/selectAdminInfo.jsp?pageNo=1&select=<%=select %>&userInfo=<%=userInfo %>">首页</a> 
               <a href="admin/adminInfo/selectAdminInfo.jsp?pageNo=<%=pageNo-1 %>&select=<%=select %>&userInfo=<%=userInfo %>">上一页</a> 
               <a href="admin/adminInfo/selectAdminInfo.jsp?pageNo=<%=pageNo+1 %>&select=<%=select %>&userInfo=<%=userInfo %>">下一页</a> 
               <a href="admin/adminInfo/selectAdminInfo.jsp?pageNo=<%=totalPage %>&select=<%=select %>&userInfo=<%=userInfo %>">末页</a>
        </center>
